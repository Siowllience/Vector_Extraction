...
ROOT_SAVE_DIR = "/data/home/jinyuehan/LLMBrain/LLMmodel/Llama/llama3.1"
# LLM-Research旗下Llama3.1的标准仓库ID（ModelScope官方命名）
LLAMA31_MODELS = {
    "8B": "LLM-Research/Meta-Llama-3.1-8B",    # 对应https://www.modelscope.cn/models/LLM-Research/Meta-Llama-3.1-8B
    "70B": "LLM-Research/Meta-Llama-3.1-70B"   # 对应https://www.modelscope.cn/models/LLM-Research/Meta-Llama-3.1-70B
}
...
for model_size, model_repo in LLAMA31_MODELS.items():
    ...
    print(f"\n{'='*50}")
    print(f"开始下载 Llama3.1-{model_size}")
    ...
    try:
        ...
        if "Failed to resolve" in str(e):
            ...
            print(f"  2. 版本确认：Llama3.1仅8B/70B等版本，无其他非标准尺寸")
        else:
...
# 最终汇总
print(f"\n{'='*50}")
print(f"所有Llama3.1版本下载流程结束！")
print(f"根路径：{ROOT_SAVE_DIR}")
for size in LLAMA31_MODELS.keys():
    print(f"  - Llama3.1-{size} 目标路径：{os.path.join(ROOT_SAVE_DIR, size)}")
print(f"{'='*50}")