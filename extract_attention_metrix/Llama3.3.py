...
model_path = "/data/home/jinyuehan/LLMBrain/LLMmodel/Llama/llama3.3/70B/LLM-Research/Meta-Llama-3-70B"
model_name = model_path.split('/')[-1]  # llama, gemma, mistral, phi-3
# model_name="Llama-3-8B"
# save_dir = f'path-to-save/{model_name}'
save_dir="/data/home/jinyuehan/LLMBrain/12.18/attention_LLM/Llama/llama3.3/70B"
...
# 修改后：强制slow tokenizer，适配Llama3.3-70B-Instruct
tokenizer = AutoTokenizer.from_pretrained(
    model_path, 
    trust_remote_code=True,
    use_fast=False,  # 关键：关闭fast tokenizer，用slow版本（解决ModelWrapper报错）
    padding_side="right",
    add_bos_token=True,
    add_eos_token=False,
    legacy=False  # 禁用旧版Tokenizer逻辑，适配Llama3.3
)
# 兜底补充pad_token（Llama3.3-Instruct默认无pad_token）
...
model = AutoModelForCausalLM.from_pretrained(
    model_path, 
    config=config,  # 传入原生配置（含完整RoPE参数）
    device_map="auto", 
    load_in_8bit=False,
    trust_remote_code=True,
